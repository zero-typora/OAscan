package ecology_WorkflowCenterTreeData_sqljni
...
func checkVulnerability(url string) bool {
	vulnUrl := url + "/mobile/browser/WorkflowCenterTreeData.jsp?node=wftype_1&scope=2333"
...
func Run(url string) {
	if checkVulnerability(url) {
		vulnUrl := url + "/mobile/browser/WorkflowCenterTreeData.jsp?node=wftype_1&scope=2333"
		color.Green.Println("[+22] 泛微OA E-Cology WorkflowCenterTreeData SQL注入漏洞#存在 -> ", vulnUrl)
	} else {
		color.Red.Println("[-22] 泛微OA E-Cology WorkflowCenterTreeData SQL注入漏洞不存在")
	}
}
