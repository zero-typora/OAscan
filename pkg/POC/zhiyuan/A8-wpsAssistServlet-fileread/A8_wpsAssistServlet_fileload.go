package A8_wpsAssistServlet_fileread
...
func checkVulnerability(url string) bool {
	...
	if err == nil && resp.StatusCode == 200 {
		...
		return strings.Contains(resp.String(), "[]")
	}
	return false
}
...
func Run(url string) {
	if checkVulnerability(url) {
		color.Green.Println("[+]	致远互联-OA wpsAssistServlet 任意文件读取漏洞存在 windows环境 -> ", url)
	} else {
		color.Red.Println("[-]	致远互联-OA wpsAssistServlet 任意文件读取漏洞不存在windows环境")
	}
}
